    ...
    getHtmlNarrative : function() {
        ...
        for(var l=0; l<this.itinData.legs.length; l++) {
            ...
            if(otp.util.Itin.isTransit(leg.mode)) { // transit
                ...
                //Stop ID ) start time
                html += '<li><b>' + _tr('Board') + '</b>: ' + leg.from.name + ' ' + _tr("(%(agency_id)s Stop ID #%(stop_id)s),", {'agency_id': leg.from.stopId.agencyId, 'stop_id': leg.from.stopId }) + ' ' + otp.util.Time.formatItinTime(leg.startTime, otp.config.locale.time.time_format) + '</li>';
                ...
                //Stop ID ) end time
                html += '<li><b>' + _tr('Alight') + '</b>: ' + leg.to.name + ' ' + _tr("(%(agency_id)s Stop ID #%(stop_id)s),", {'agency_id': leg.to.stopId.agencyId, 'stop_id': leg.to.stopId }) + ' ' + otp.util.Time.formatItinTime(leg.endTime, otp.config.locale.time.time_format) + '</li>';
    ...
    getTextNarrative : function(itinLink) {
        ...
        for(var l=0; l<this.itinData.legs.length; l++) {
            ...
            if(otp.util.Itin.isTransit(leg.mode)) {
                //TRANSLATORS: Board Public transit route name (agency name
                //Stop ID ) start time
                text += ' - ' + _tr('Board') + ': ' + leg.from.name + ' ' + _tr("(%(agency_id)s Stop ID #%(stop_id)s),", {'agency_id': leg.from.stopId.agencyId, 'stop_id': leg.from.stopId }) + ' ' + otp.util.Time.formatItinTime(leg.startTime, otp.config.locale.time.time_format) + '\n';
                text += ' - ' + _tr('Time in transit') + ': '+otp.util.Time.secsToHrMin(leg.duration) + '\n';
                //TRANSLATORS: Alight Public transit route name (agency name
                //Stop ID ) end time
                text += ' - ' + _tr('Alight') + ': ' + leg.to.name + ' ' + _tr("(%(agency_id)s Stop ID #%(stop_id)s),", {'agency_id': leg.to.stopId.agencyId, 'stop_id': leg.to.stopId }) + ' ' + otp.util.Time.formatItinTime(leg.endTime, otp.config.locale.time.time_format) + '\n';
            }
            else if (leg.steps) { // walk / bike / car
